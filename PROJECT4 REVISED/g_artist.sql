...
PROCEDURE update_artwork (
  ...
  l_count NUMBER;
BEGIN
  SELECT COUNT(*) INTO l_count FROM g_admin.ARTWORK WHERE ArtworkID = p_artwork_id;

  IF l_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001, 'Artwork does not exist');
  END IF;

  UPDATE g_admin.ARTWORK SET
    UserID = p_user_id,
...
BEGIN
  ...
  DELETE FROM g_admin.ARTWORK WHERE ArtworkID = p_artwork_id;
END delete_artwork;
END artist_actions_pkg;
/
...
BEGIN
  ...
  SELECT MAX(ArtworkID) INTO l_artwork_id FROM g_admin.ARTWORK;
...
BEGIN
  ...
  SELECT ArtworkID INTO l_artwork_id FROM g_admin.ARTWORK WHERE ArtworkID = 11;
