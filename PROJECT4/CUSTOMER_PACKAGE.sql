...
PROCEDURE place_order (
    ...
    v_shipping_address ORDERS.ShippingAddress%TYPE;
    v_total_amount ORDERS.TotalAmount%TYPE;
BEGIN
    IF o_order_id IS NOT NULL THEN
            p_msg := 'Order id should be null for update action';
            RETURN;
    END IF;
    select dbms_random.value(100000000000, 999999999999) into v_transaction_id  from dual;
      ...
      SELECT (ADDRESSLINE1 || ADDRESSLIN2 || CITY || STATE || COUNTRY || ZIPCODE) INTO v_shipping_address from CONTACT WHERE USERID= o_user_id;
    ...
    VALUES (orders_seq.NEXTVAL, o_user_id, 1, v_transaction_id, v_transaction_method, v_transaction_status, v_order_status, v_shipping_status, p_shipping_address, SYSDATE, v_total_amount);
END;
COMMIT;
