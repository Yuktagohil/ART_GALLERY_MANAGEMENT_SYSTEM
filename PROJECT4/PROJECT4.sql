...
CREATE OR REPLACE FUNCTION manage_users (
    ...
    p_speciality IN VARCHAR2 DEFAULT NULL,
    ...
    p_zipcode IN NUMBER DEFAULT NULL,
...
BEGIN
    IF (p_action = 'ADD') THEN
        ...
        v_msg := 'User added successfully';
    ELSIF (p_action = 'UPDATE') THEN
        UPDATE USERS SET RoleID = p_role_id, EmailID = p_email_id, UserName = p_username, Password = p_password, FirstName = p_firstname, LastName = p_lastname, Speciality = p_speciality, Nationality = p_nationality
        WHERE UserID = p_user_id;

        UPDATE CONTACT SET ContactNumber = p_contact_number, AddressLine1 = p_address_line1, AddressLine2 = p_address_line2, City = p_city, State = p_state, Country = p_country, ZipCode = p_zipcode
        WHERE UserID = p_user_id;
    ...
    ELSIF (p_action = 'DELETE') THEN
        ...
        v_msg := 'User deleted successfully';
    ELSE
        v_msg := 'Invalid action';
    END IF;
